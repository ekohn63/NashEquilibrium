...
import numpy as np
from model.action_set import PitcherAction
...
def test_tightness():
    ...
    y_val, p_val, primal = pyomo_primal(A, E, F)
    x_val, q_val, dual_val, y_from_dual, p_from_dual = pyomo_dual(A, E, F)
    x_val_br, p_val, val_br = pyomo_batter_br(A, E, y_val)

    behavioural = realization_to_behavioural(y_val, pitchersequences, F)

    print(f"primal: {primal}, br_val: {val_br}")
    print(f"y_val: {y_val}")
    print(f"x_val_br: {x_val_br}, x_val: {x_val}")
    print(f"behavioural: {behavioural}")

    print(f"behavioural[17]: {behavioural[17]}")
    behavioural[17][PitcherAction.Fastball_Top] = 1
    behavioural[17][PitcherAction.Offspeed_Chase] = 0

    print(f"modified behavioural: {behavioural}")
    realization = behavioural_to_realization(behavioural, pitchersequences)

    x_val_br2, p_val, val_br2 = pyomo_batter_br(A, E, realization)

    print(f"val_br2: {val_br2}, val_br: {val_br}")
